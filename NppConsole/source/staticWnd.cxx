...
LRESULT WINAPI CStaticWnd::KeyBoardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (WM_KEYDOWN == wParam) {
		if (VK_LCONTROL == hs.vkCode && hs.flags == (hs.flags & !LLKHF_INJECTED)) {
			bCtrlDown = true;
		}
		// 0x43 - C
		if (0x43 == hs.vkCode && (hs.flags == 0 || hs.flags == LLKHF_INJECTED) && bCtrlDown) {
			CStaticWnd *pWnd = (CStaticWnd*)::GetWindowLongPtr(s_hWnd, GWLP_USERDATA);
...
BOOL CStaticWnd::CreateConsoleProcess(LPCTSTR cmd)
{
	...
	if (param) {
		if (_tcslen(param) > 3) {
			...
			if (paramEnd){
				...
				if (MAX_PATH > paramLen - paramEndLen && _tcsncpy(tmpParam, param,  (paramLen - paramEndLen))) {
					paramTmp = tmpParam;
	...
	if (!CreateProcess(cmdTmp.c_str(), paramTmp.length()?tmpParam:NULL,NULL,NULL,FALSE,NORMAL_PRIORITY_CLASS,NULL,NULL,&si, &m_pi)) {
		cmdTmp = m_cmd;
		paramTmp = m_param;
		IFR(!CreateProcess(cmdTmp.c_str(), paramTmp.length()?tmpParam:NULL,NULL,NULL,FALSE,NORMAL_PRIORITY_CLASS,NULL,NULL,&si, &m_pi), NULL);
	}
	for (DWORD ms=500; ms<3000 && !htest; ms+=500) {
		Sleep(ms);
		htest=OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, m_pi.dwProcessId);
	}
	IFR(!htest, FALSE);
	CloseHandle(htest);
	m_cmd = cmdTmp;
